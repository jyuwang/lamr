...
def sortGtConstraints (x : String)  (exps : List LinearExp) :
  ...
  exps.foldl (fun (noX, pos, neg) exp => Id.run do
    if exp.contains x then
      let a := exp.find! x
      if a > 0 then
        pure (noX, exp :: pos, neg)
      else
        pure (noX, pos, exp :: neg)
    else
      pure (exp :: noX, pos, neg))
  ([], [], [])
...
def elimVarGtConstraints (x : String) (gtzeros : List LinearExp) : Option (List LinearExp) := Id.run do
  ...
  let mut newConstraints : List LinearExp := noX
  for p in pos do
    for n in neg do
      let a := p.findD x 0
      let b := n.findD x 0
      if a != 0 && b != 0 then
        ...
        if newConstraint.isEmpty then
          ...
          newConstraints := newConstraint :: newConstraints
  some newConstraints
